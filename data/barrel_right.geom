...
uniform float uDistortionOffset = 0.151976;
...
*/
  ...
  sTexCoords = vec2( coords.z, coords.w);
  EmitVertex();

  gl_Position = vec4(screen.x, screen.w, 0.0, 1.0 );
  sTexCoords = vec2( coords.x, coords.w );
  EmitVertex();

  gl_Position = vec4(screen.z,screen.y, 0.0, 1.0 );
  sTexCoords = vec2( coords.z, coords.y );
  EmitVertex();

  gl_Position = vec4(screen.x,screen.y, 0.0, 1.0 );
  sTexCoords = vec2( coords.x, coords.y );
  EmitVertex();
